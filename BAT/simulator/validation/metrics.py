...
def compile_metrics(hist_data: pd.DataFrame) -> Tuple[float, float]:
    ...
    data = (
        ...
        .agg(
            ...
            tvc_history=('tvc', 'last'),
            cpc_percent_history=('cpc_percent', 'last'),
            cpc_avg_up_history=('cpc_avg_up', 'last'),
            cpc_avg_down_history=('cpc_avg_down', 'last'),
        )
    )

    tvc = data['tvc_history'].mean()  # sum for i in I
    cpc_percent = data['cpc_percent_history'].mean() * 100  # mean for i in I
    cpc_avg = (data['cpc_avg_up_history'] / data['cpc_avg_down_history']).mean()  # sum for i in I / sum for i in I
    # cpc_avg = np.divide(data['cpc_avg_up_history'], data['cpc_avg_down_history'],
    #               where=data['cpc_avg_down_history'] != 0).mean()
    return (tvc, cpc_percent, cpc_avg), data

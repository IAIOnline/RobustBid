...
class RobustBidMseCtrcvr(_Bidder):
    """RobustBidMseCtrcvr class."""
    default_params = {
    ...
    def __init__(self, params: dict = None):
        ...
        self.T_history = params.get('T', self.default_params['T'])

        self.alpha = sqrt(2 * eps_ctr)

    def place_bid(self, bidding_input_params, history: History) -> float:
        """place_bid."""
        n_transactions = min(bidding_input_params['T'], self.T_history)
        ...
        if len(history.rows):
            lambda_ctr, lambda_cvr = self.solve_for_lambda(ctr_list, cvr_list,
                                                           ...
                                                           self.C, n_transactions, self.r_ctr, self.r_cvr,
    ...
    def calculate_dual_params(
        self, ctr_list, cvr_list, wp_list, a_t0, b_t0, budget, n_transactions, lambda_ctr, lambda_cvr
    ):
        """calculate_dual_params."""
        def objective(params, *args):
            gamma, u_0 = params
            budget, n_transactions, ctr_list, cvr_list, wp_list, alpha, C, lambda_ctr, lambda_cvr = args
            sum_max = 0
        ...
        args = (
            budget, n_transactions, ctr_list[:n_transactions], cvr_list[:n_transactions],
            wp_list[:n_transactions], alpha, C, lambda_ctr, lambda_cvr
        )
    ...
    def solve_for_lambda(
        ctr_list, cvr_list, wp_list, x_list, _budget, _cost, n_transactions, r_ctr, r_cvr, last_lambda_a, last_lambda_b
    ):
        """solve_for_lambda."""
        ctr_list = np.array(ctr_list)[:n_transactions]
        cvr_list = np.array(cvr_list)[:n_transactions]
        wp_list = np.array(wp_list)[:n_transactions]
        x_list = np.array(x_list)[:n_transactions]
